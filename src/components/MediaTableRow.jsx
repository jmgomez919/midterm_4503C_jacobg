...
// MediaTableRow renders one item as a <tr> inside the table view.
// In edit mode it expands to a full-width edit form spanning all columns.
// Props mirror MediaCard: item, isFavorite, toggleFavorite, onUpdate, onDelete
function MediaTableRow({ item, isFavorite, toggleFavorite, onUpdate, onDelete }) {
  ...
  // ── Edit mode: full-width row spanning all columns ──────────
  if (editing) {
    return (
      <tr className="table-row-editing">
        <td colSpan={5} className="table-edit-cell">
  ...
  return (
    <tr className={`table-row ${isFavorite ? 'table-row-favorited' : ''}`}>
      ...
      <td className="table-cell-actions">
        ...
        {onUpdate && (
          <button className="btn-edit btn-edit-sm" onClick={handleEdit} title="Edit">
            Edit
          </button>
        )}
      </td>
